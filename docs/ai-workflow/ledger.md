...
Task 1: review clean; ⚠️ trailer confirmed via git log; Ruling: `-c commit.gpgsign=false` is a no-op (no signing configured locally or globally) — dropped from future dispatches — cost if wrong: none, commits are unsigned either way.
