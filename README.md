...
VaultRadar was built with **Claude Code** under human direction, using a spec-driven
subagent workflow: a written spec, a written plan, one implementer per task working from a
written brief, an independent reviewer for each task's diff, and a whole-branch review with
a fix wave at the end. Practically all of the code, tests, documentation and diagrams were
generated by the tool; the human contribution was direction, decisions and credentials —
the project and track selection, the design approval, the scope cuts, every account and key,
...
wrong, and it does not flatter the outcome.
